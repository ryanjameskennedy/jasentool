...
class OptionsParser(object):
    ...
    def _get_output_fpaths(self, input_files, output_dir, output_file, prefix):
        output_fpaths = []
        if output_dir:
    ...
    def validate(self, options):
        Database.initialize(options.db_name)
        input_files = self._input_to_process(options.input_file, options.input_dir)
        output_fpaths = self._get_output_fpaths(input_files, options.output_dir, options.output_file, options.prefix)
        validate = Validate()
        validate.run(input_files, output_fpaths, options.db_collection)
